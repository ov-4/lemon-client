...
public class CityBreaker extends LemonModule {
   ...
   public final Setting autoSwapSetting;
   private final Setting swapBack;
   private final Setting support;
   private final Setting instant;
   private final Setting instantDelay;
   private final Setting test;
   ...
   private final Setting progressScale;
   ...
   public CityBreaker() {
      ...
      this.targetRange = this.sgGeneral.add(((DoubleSetting.Builder)((DoubleSetting.Builder)(new DoubleSetting.Builder()).name("target-range")).description("The radius in which players get targeted.")).defaultValue(6.0).min(0.0).sliderMax(7.0).build());
      ...
      EnumSetting.Builder var10002 = (EnumSetting.Builder)((EnumSetting.Builder)(new EnumSetting.Builder()).name("swap-mode")).defaultValue(CityBreaker.swapMode.Normal);
      ...
      this.sideColor = this.sgRender.add(((ColorSetting.Builder)((ColorSetting.Builder)(new ColorSetting.Builder()).name("side-color")).description("The side color for positions to be placed.")).defaultValue(new SettingColor(255, 0, 0, 75)).build());
      this.lineColor = this.sgRender.add(((ColorSetting.Builder)((ColorSetting.Builder)(new ColorSetting.Builder()).name("line-color")).description("The line color for positions to be placed.")).defaultValue(new SettingColor(255, 0, 150)).build());
      ...
      this.progressScale = this.sgRender.add(((DoubleSetting.Builder)((DoubleSetting.Builder)(new DoubleSetting.Builder()).name("progress-scale")).description("scale of text")).defaultValue(1.5).min(0.0).sliderMax(3.0).build());
      this.progressColor = this.sgRender.add(((ColorSetting.Builder)((ColorSetting.Builder)(new ColorSetting.Builder()).name("progress-color")).description("the color of text rendered")).defaultValue(new SettingColor(255, 255, 255)).build());
   }
   ...
   private void onTick(TickEvent.Pre event) {
      ...
      if (TargetUtils.isBadTarget(this.target, (Double)this.targetRange.get())) {
         ...
         this.blockPosTarget = null;
         this.breakTimer = 0;
         ChatUtils.error("No target founded! Disabling...", new Object[0]);
         this.toggle();
      } else {
         ...
         if (this.delay() == 0) {
            ChatUtils.error("No pick in hotbar! Disabling...", new Object[0]);
            this.toggle();
         } else if (this.blockPosTarget == null) {
            ChatUtils.error("No city block found! Disabling...", new Object[0]);
            this.toggle();
         } else {
            if (PlayerUtils.distanceTo(this.blockPosTarget) > (double)this.mc.field_1761.method_2904()) {
               ChatUtils.error("Block to mine too far! Disabling...", new Object[0]);
            }
            ...
            if (!this.antiSpam) {
               this.info("breaking " + this.target.method_7334().getName() + " surround", new Object[0]);
               if ((Boolean)this.support.get()) {
                  class_2338 downPos = this.blockPosTarget.method_10087(1);
                  Managers.ROTATION.start(downPos, (double)this.priority, RotationType.BlockPlace, (long)Objects.hash(new Object[]{this.name + "placing"}));
                  BlockUtils.place(downPos, InvUtils.findInHotbar(new class_1792[]{class_1802.field_8281}), 0, true);
               }

               if (this.autoSwapSetting.get() == CityBreaker.swapMode.Normal) {
                  this.pickSwap();
               }

               if (SettingUtils.shouldRotate(RotationType.Mining)) {
                  Managers.ROTATION.start(this.blockPosTarget, (double)this.priority, RotationType.Mining, (long)Objects.hash(new Object[]{this.name + "placing"}));
   ...
   private void onRender2D(Render2DEvent event) {
      if (this.blockPosTarget != null && (Boolean)this.renderProgress.get() && this.target != null && !this.mc.field_1724.method_31549().field_7477) {
         Vector3d pos = new Vector3d((double)this.blockPosTarget.method_10263() + 0.5, (double)this.blockPosTarget.method_10264() + 0.5, (double)this.blockPosTarget.method_10260() + 0.5);
   ...
   public static enum swapMode {
      ...
      private static swapMode[] $values() {
         return new swapMode[]{Normal, Silent};
      }
   }
}
